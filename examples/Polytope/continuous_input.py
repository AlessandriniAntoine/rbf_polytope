...
print("T:\n", T)
A = T @ vertices @ np.block([[T.T, np.zeros_like(T), np.zeros_like(T)], [np.zeros_like(T), T.T, np.zeros_like(T)], [np.zeros_like(T), np.zeros_like(T), T.T]])
...
u_train = lambda t: np.sin(t/10).reshape(-1, 1)
u_test = lambda t: np.sin(t/5).reshape(-1, 1)
...
def polytopic_model(t, x, vertices, centers, widths, C, u_fct):
    u = u_fct(t)
    x = x.reshape(-1, 1)
    y = C @ x
...
sol_train = solve_ivp(polytopic_model, t_span, x0_train.flatten(), t_eval=t_eval, args=(vertices, centers, widths, C, u_train))
sol_test = solve_ivp(polytopic_model, t_span, x0_test.flatten(), t_eval=t_eval, args=(vertices, centers, widths, C, u_test))
X_train, X_test = sol_train.y.T, sol_test.y.T
dX_train = np.array([polytopic_model(t, x, vertices, centers, widths, C, u_train) for t, x in zip(sol_train.t, X_train)])
dX_test = np.array([polytopic_model(t, x, vertices, centers, widths, C, u_test) for t, x in zip(sol_test.t, X_test)])
...
X_int_train_linear = solve_ivp(linear_model, t_span, x0_train.flatten(),
                               t_eval=t_eval, args=(state_matrix,u_train)).y.T
X_int_test_linear = solve_ivp(linear_model, t_span, x0_test.flatten(),
                              t_eval=t_eval, args=(state_matrix, u_test)).y.T
...
X_int_train = simulate(model, x0_train, C, t_span=t_span, t_eval=sol_train.t, command_fct=u_train)
X_int_test = simulate(model, x0_test, C, t_span=t_span, t_eval=sol_test.t, command_fct=u_test)
